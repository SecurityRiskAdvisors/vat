...
func TestRoundtripAssessmentData(t *testing.T) {
	...
	s, _, _ := util.SetupVectrClient(src_hostname, src_creds, &util.CustomTlsParams{InsecureConnect: true})
	d, _, _ := util.SetupVectrClient(dst_hostname, dst_creds, &util.CustomTlsParams{InsecureConnect: true})
